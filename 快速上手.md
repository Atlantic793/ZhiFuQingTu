...
| 个人中心 | `/profile` | 用户信息可编辑（头像/昵称/地址/GitHub/简介）；学习数据仍多为静态展示 |
...
1. 在 [Supabase](https://supabase.com) 使用**团队共用的云项目**（向负责人要 Project URL + anon key）  
2. 复制 `.env.example` 为 `.env`，填入 `VITE_SUPABASE_URL` 与 `VITE_SUPABASE_ANON_KEY`  
   - **不要**写成 `http://localhost:54321` 或 `VITE_DEMO_MODE`（当前仓库无 demo 模式；`.env` 不进 Git，每人本地自建）  
3. 在 SQL Editor **依次**执行：  
   ...
   - `supabase/migrations/20260328000003_profile_fields.sql`（个人主页可编辑字段 + 头像 Storage）  
...
**编目 vs 会话：** `Rating`/`Training` 读编目失败时可回退 `mockData`（页面仍能看）；Agent 会话/消息**必须**走 Supabase，失败会报错而不是假会话——这是预期行为。
