...
import { ChevronDown, Send, Search, ExternalLink, Star } from 'lucide-react';
...
const CourseChat = () => {
  ...
  const switchToContext = useCallback(
    async () => {
      ...
      try {
        ...
        const group = conversationsInContext(list, 'courses', null);
        let conv: Conversation;

        if (group.length > 0) {
          conv = group[0];
        } else {
          const title = buildGroupTitle('courses', null, group.length + 1);
          ...
          await insertMessage({
            ...
            userId: user.id,
            role: 'assistant',
            content: modeIntro('courses', null),
            payload: { intro: true },
          });
        }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const reply = await chatWithGLMStream(text, null, history, 'courses', {
        onStatus: (event) => {
